...
__global__ void NeedlemanWunschSemiglobalScoreKernel(short2* AF, int* scores, bool border = false)
{
    ...
    {        
        ...
        for (short x = 0; x < lengthXY.x; x++)
        {
            ...
            short2 SimF_current = AF[startPosA + x * MEM_OFFSET];
            ...
            //search for max value in last row of A
            if( (y + ymin_score.x) == lengthXY.y )
                ymin_score.y = max(ymin_score.y, AE_current.x);
                

        }

        //search for max value in last column of A
        for(short i = 0; i < ymin_score.x; i++)
            ymin_score.y = max(ymin_score.y, AE_shared[i][blockThread].x);
...
const char* NeedlemanWunschSemiglobalScoreGpu::getAlgorithmName()
{
    return "NeedlemanWunschSemiglobalScoreGpu";
}

void NeedlemanWunschSemiglobalScoreGpu::kernelInvocation(void* kernelParams)
{
    KernelScoreInvokingParams* params = (KernelScoreInvokingParams*) kernelParams;
    NeedlemanWunschSemiglobalScoreKernel<<<params->gridShape,params->blockShape>>>(params->AF, params->scores, params->border);
}
